...
class ViewAdjustAgent:
    ...
    def llm_view_adjust(self, scene, message):
        try:
            ...
            #############################################
            if "{" not in answer:
                print(colored("[Warning] No JSON found in LLM response. Defaulting to zero transformation.", "red"))
            ...
            #############################################
            ###############################################
            if not isinstance(delta_extrinsic, dict):
                print(colored("[Warning] Parsed response is not a dictionary. Using default values.", "red"))
                return {'x': 0, 'y': 0, 'z': 0, 'theta': 0}
            #################################################
            ####################################################
